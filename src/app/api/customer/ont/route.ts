...
export async function GET(request: NextRequest) {
  try {
    ...
    // Voucher users don't have ONT devices
    if (session.userId.startsWith('voucher_')) {
      return NextResponse.json({
        success: true,
        device: null,
        message: 'Voucher users do not have ONT devices',
      });
    }
    ...
    if (!response.ok) {
      return NextResponse.json({
        success: true,
        device: null,
        message: 'GenieACS API unavailable',
      });
    }

    const allDevices = await response.json();

    // Filter devices by PPPoE username
    ...
    return NextResponse.json({
      success: true,
      device: null,
      message: 'Error loading ONT device',
    });
  }
}