...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.Body && body.Body.stkCallback) {
      ...
      if (callback.ResultCode === '0') {
        ...
        amount = amountItem?.Value
          ? parseInt(String(amountItem.Value))
          : undefined;
      } else {
        status = 'failed';
      }
    ...
    // ANYPAY
      ...
      payload.order_id &&
      (payload.status || payload.anypay_payment_status)
    ) {
      gateway = 'anypay';
      orderId = payload.order_id;
      transactionId =
        payload.transid || payload.reference || payload.anypay_transid || '';
      ...
      console.log(
        'Unknown webhook payload format. Returning 200 to avoid retries.'
      );
...
async function handleVoucherOrder(
  ...
  console.log(
    `✅ Generated ${vouchers.length} vouchers for order ${order.orderNumber}`
  );
  ...
  try {
    ...
    console.log(
      `✅ WhatsApp voucher notification sent to ${order.customerPhone}`
    );
  } catch (waError) {
...
async function handleInvoicePayment(
  ...
  try {
    ...
    if (pppoeCategory) {
      ...
      if (!existingTransaction) {
        const customerName =
          invoice.customerName || invoice.user?.name || 'Unknown';
        ...
        `;
        console.log(
          `✅ Transaction synced to Keuangan: ${invoice.invoiceNumber} (${formatCurrency(
            invoice.amount
          )})`
        );
      }
  ...
  console.log(
    `   - Expiry: ${
      user.expiredAt?.toISOString() || 'N/A'
